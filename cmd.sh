...
ssh -i ${KEY_PATH} -N -o StrictHostKeyChecking=no -o ProxyCommand="socat - PROXY:${PROXY_HOST}:%h:%p,proxyport=${PROXY_PORT}" -R ${REMOTE_PORT}:${TARGET_IP}:${TARGET_PORT} -N -f ${GW_USER}@${GW_HOST}
